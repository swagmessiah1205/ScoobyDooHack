	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		g_main_thread = CreateThread(
		    ...
		    [](PVOID) -> DWORD {
			    ...
			    base_dir /= "ScoobyDooHack";
			    g_file_manager.init(base_dir);

			    g.init(g_file_manager.get_project_file("./settings.json"));
			    g_log.initialize("ScoobyDooHack", g_file_manager.get_project_file("./cout.log"), g.debug.external_console);
			    LOG(INFO) << "Settings Loaded and logger initialized.";

			    LOG(INFO) << "ScoobyDooHack Initializing";
